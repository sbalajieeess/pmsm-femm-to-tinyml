...
fem_file = r"C:/Work/FEMM_Server/1K2_FemmFiles/1k2-50mm-7T/1k2w_halfSymmetry.FEM" # stack length of the motor has to be set in the file correctly
...
def main():
    if __name__ ==  '__main__': 
        ...
        settings["dAxis_slidingBand"] = 3
        settings["DC_Voltage"] = 48
        settings["RMSCurrent"] = 0
        settings["phaseAdvanceAngle"] = 0
        settings["Symmetry"] = 2
        settings["polePairs"] = 5
        settings['noOfSlices'] = 1
        settings['skew_AnglesMech'] = [0]
        ...
        cpu_count = 1 # has to be same as no of slices.
        
        #create a folder to put all the results in
        saveDirectory = rootFolder +"\\SkewOutputs_{}Slices\\OL\\".format(settings['noOfSlices'])
        ...
        #start from Iq = 160
        Iqs = list(np.arange(10,200,10))
        Ids = list(np.arange(0,-200,-10))
