...
fem_file =  r"C:/Work/Projects/LTTS_Work/design6_152mm/one_eighth_v6_100C.FEM"
...
Symmetry= 8
ParallelPaths = 4
windingsymmetry = Symmetry/ParallelPaths
polePairs = 4
...
if __name__ ==  '__main__': 
    ...
    if PROCESS_RESULTS:
        ...
        if SAVE_IMAGES:
            ...
            plt.savefig(imgName)
            print ("plot image of single slive FL and dFL saved in {}".format(imgName))
            
         
        #2.Summing all the FLs and finding the dAxis rotorAngle
        # even if we re parallel paths, the paralleling is not happening across the slices. So we need to add the FL's
        # However, each final FL has to be multiplied by windingsymmetry to get the final FL
        totalFL_unscaled = 0
        total_FL_scaled = 0
        for LF in slice_FLs:
            totalFL_unscaled = totalFL_unscaled + LF
        total_FL_scaled = totalFL_unscaled * windingsymmetry
        
        maxIdx = int(np.argmax(totalFL_unscaled))
        maxAngle = rotorAngles.iloc[maxIdx]
        maxPhaseFluxValue =np.max(totalFL_unscaled)
        
        print ("winding Symmetry = {}".format(windingsymmetry))
        ...
        print('------------')
        if Symmetry:
            print("maxU phase flux Value = " + str(maxPhaseFluxValue *windingsymmetry))
            BackEMF_Vll_RMS = (maxPhaseFluxValue* windingsymmetry) * 104.72 * polePairs * np.sqrt(3)/np.sqrt(2) # 
        else:
        ...
        phaseBackEMF = total_dFL_unscaled/time_for_each_step * windingsymmetry
